...
class LeCaR_New:
    ...
    def getRand(self, r):
        _, val = random.choice(list(r.items()))
        return val
    ...
    def adjustWeights(self, rewardLRU, rewardLFU, rewardRAND):
        reward = np.array([rewardLRU, rewardLFU, rewardRAND], dtype=np.float32)
    ...
    def miss(self, oblock):
        ...
        if oblock in self.lru_hist:
            ...
            self.adjustWeights(reward_lru, 0, 0)
            ...
            reward_rand = -(self.discount_rate
                           **(self.time - entry.evicted_time))
            self.adjustWeights(0, 0, reward_rand)
